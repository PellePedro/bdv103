use('bookstore');
//Select the database to use.use('bookstore');

// Create the "books" collection
db.createCollection("books");

// Insert the books from mcmasteful-book-list.json
db.books.insertMany([
  {
    "name": "Giant's Bread",
...
// Filter books between $20 and $40
const filteredBooks = db.books.find({
  price: { $gte: 20, $lte: 40 }
}).toArray();

// Print the filtered books
print("Books between $20 and $40:");
filteredBooks.forEach(book => {
  print(`${book.name} by ${book.author} - $${book.price}`);
});

// Print the count of filtered books
print(`Number of books between $20 and $40: ${filteredBooks.length}`);

// Print the number of documents inserted
print("Number of books inserted: " + db.books.countDocuments());

// Optional: Print the first document to verify insertion
